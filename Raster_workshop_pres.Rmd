...
Packages: `sp`, `rgdal`, and `rgeos`

sp: importing, manipulating and exporting spatial data in R, and methods

rgdal:  bindings to parts of the **OGR Simple Feature Library**
    - variety of vector file formats including ESRI Shapefiles and kml. The OGR library is part of the widely used Geospatial Data Abstraction Library (GDAL). The GDAL library is the most useful freely-available library for reading and writing geospatial data. The GDAL library is well-documented (http://gdal.org/), but with a catch for R and Python programmers. The GDAL (and associated OGR) library and command line tools are all written in C and C++. Bindings are available that allow access from a variety of other languages including R and Python but the documentation is all written for the C++ version of the libraries. This can make reading the documentation rather challenging. Fortunaly, the rgdal package, providing GDAL bindings in R, is well documented with lots of examples. The same is valid for the Python libaries.

Similarly, rgeos is an interface to the powerful Geometry Engine Open Source (GEOS) library for all kinds of operations on geometries (buffering, overlaying, area calculations, etc.).We will use the `rgdal` package to work with vector data in `R`. Notice that the
